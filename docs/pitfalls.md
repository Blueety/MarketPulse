...
- **CYB（`399006.SZ`）Yahoo 1y 历史覆盖缺口**：同一条 `range=1y` 请求，`^GSPC`/`^IXIC` 各 252 天、`000001.SS`/`399001.SZ` 各 243 天，而 `399006.SZ` **只返回 1 天** → 回填段 CYB 全空（`/api/history?days=365` 中 `cyb` 仅 86/259 非空）。属外部数据源缺口（非本地 bug）；要补需改走 AkShare `stock_zh_index_daily(symbol="sz399006")`。
