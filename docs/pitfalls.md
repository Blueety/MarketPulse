...
## 通用（走查/评审报告的引用必须 grep 实证，2026-09-17）

- **走查/评审报告里点名的 token 名、类名、函数名必须先 `grep` 实证再实施**：本次走查报告 P1-① 写
  "加深 `--text-tertiary`" —— 仓库里**根本没有**这个 token，同类语义的 token 叫 **`--text-muted`**
  （light `#9CA3AF` / dark `#8E9BAE`）。照报告的 token 名去改，要么新增一个没人消费的变量（假修复），
  要么改错对象。修法：实施前 `grep -rn "报告里的名字" web/`，命中为零就改按**语义**找真身，并把
  "报告用错名"写进任务档（本次同时发现：**dark 侧早已单独修过**（`#8E9BAE` ≈6.8:1，`style.css:53-54`
  有注释留档），"深色修了、浅色没修"的不对称才是真问题）。
- **修对比度时的连带面必须先数清楚**：`--text-muted` 这类全局 token 的消费点约 30 处
  （`.ms-time/.link-btn/.ph-note/.empty/.mac-note/.mac-empty/.score-cap/...`），其中一部分是
  **装饰用途**（`.kpi-card::before` 色条 / `.ms-dot` / `.h-bar i.neutral`）——加深 token 会把它们一起加深。
  这是**有意接受**的连带（dark 侧此前做同款取舍时已留注释），但必须**双主题目视截图留档**，不能只看数字达标。
- **系列色被当文字色用时，按"light 加深 / dark 提亮"的既有模式改 token**：`.chart-meta` 的涨跌文字色
  直接取系列色（`app.js renderTrendMeta` 的 `span.style.color = palette[s.key]`）⇒ light 的
  `--c-ixic: #13C2C2`（青）在白卡上仅 2.08:1。修法：light 段加深为同色相的 **#0D7D7D**（4.61~4.95:1），
  dark 段 `#2FD6A8` 不动（深底 10.35:1）——与 `--green` 的"light `#16A085` / dark `#2FD6A8`"同一模式。
  **选值必须用 WCAG 公式算**（本仓库用 `relative luminance + (L1+0.05)/(L2+0.05)`），别目测。

## 通用（产线走查报告的裁定：先分清「有意占位」与「故障」，2026-09-17）

- **走查报告说"坏了"的东西，先分清是「有意占位」还是「真缺陷」——两者修法完全不同**：产线走查报告把
  「全球市场动态未接入」「资金流向未接入」列为 P0 —— 但这两个是**有意设计的静态占位**
  （`#promo-global` 从无数据源、JS 不碰它；资金流向是全站唯一 `data-placeholder="1"` 且验收断言钉死计数=1），
  给它们加骨架屏/重试/错误态都是**伪修复**。裁定依据要落到代码证据：`grep -rn data-placeholder web/`、
  `grep -rn "promo-global" web/static/app.js`（零消费 = 无数据源）。**判据**：走查项定为缺陷前，先回答
  "这个位置设计上应该有数据吗"，答不出来就去翻 plan/architecture 的占位清单（`frontend-structure.md §8`）。
- **报告里的"多出一列"类观感问题，先核对表头与渲染代码，再决定动不动**：走查报告称"自选列表多出一列红"，
  实测列**不是错位** —— 表头本来就有留空的 `<th class="col-ico">`，"红"是图标 fallback 取名称首字
  （`app.js:932` 「红利低波ETF」→「红」）。真正可修的是**空表头无语义标注**（`aria-label`），
  而列结构/`colspan`/图标 fallback 都是既有断言依赖，不能动。
- **给"滚动容器内的卡片"加字，必须先实测卡片高度**：告警/资讯是自动滚动容器（内容不足一屏时**不得启动**，
  否则原地抖动 —— B-12 护栏）。首版 G1 措辞带全日期 ⇒ 单条卡片 134px > 容器 132px ⇒ 滚动被启动
  ⇒ B-12a/b 新增红。**做法**：用探针把候选措辞逐个注入 DOM 测卡片高度（`%TEMP%\mp_g1_fit.py` 的做法），
  选"放得下且信息量最大"的；日期这类锚点若放不下，交给卡片头部承载（头部本来就有）。
...
## 数据层（外部日程源 + 交易日口径，2026-09-18）

- 🔴 **`history` 表在休市日带"沿用值" ⇒ 按行序算 `+h 交易日` 会得到假的 0.00%**：实测形态
  ```
  ...
  ```
  用 `row[i + h]`（`scripts/backtest.py` 的既有口径）会把"沿用行"当交易日：09-04 的 `sh` 在 +1 得到
  **`0.0`**（假），此后每个窗口整体错位一天；"当日涨跌"同理 —— 周一会拿周日的沿用值当"前收"。
  **修法（`src/timeline.py`）**：给每个标的建**交易日轴** = 只保留"非空且与前一个保留值不同"的行，
  再在轴上取 `pos + h`。"与前值相同"这条规则同时覆盖周末沿用与**工作日休市**（劳动节），
  且各标的各算各的轴（美股/A 股休市不同步互不污染）。代价：真正的"零变动日"会被并掉
  （指数两位小数下几乎不可能）。**判据**：任何"按行序推进 N 根"的行情口径，先确认表里有不有"沿用行"。
- 🔴 **解析外部日程源：三类别名/形态坑**（本轮实测，全部会静默出错）：
  ① **月份混用全名与缩写** —— Fed FOMC 页普通行写 `January`，**跨月会议写 `Jan/Feb` + 日期 `31-1`**
  （2023 两场、2024 一场）。只认全名 → **整场会议被静默跳过**（初版漏 3 场）。发现手法：
  逐块把"原始 anchor 数"与"解析条数"对账（`fomc-meeting__date` 出现次数 vs 解析结果）。
  ② **`STATUS:CANCELLED` 占 75/185（40%）**，且**标题后缀与 STATUS 是两条独立路径**
  （64 条带 `[CANCELLED]` 后缀、11 条只有 STATUS）⇒ 两条都要滤。`TENTATIVE` 保留并标注（是有效信息）。
  ③ **同一事件跨源/同源重复** —— 镜像内部"同一天两条非农 / 两条 PPI"，且 FOMC 家族被拆成
  `Day 1` / `Day 2` / `Rate Decision` / `Press Conference` / `Minutes`（同一次会议 3~4 条、Day1 与 Day2
  还是不同日期）⇒ 先按**源分工**（FOMC 一律走官方页、镜像的 FOMC 家族整族丢弃），再按 `(date, kind)`
  归一化去重（`normalize_kind` 未命中要记 warning 归 `其他`，不能静默丢）。
  **判据**：接入任何"别人维护的日程/清单"前，先跑一次"原始条目数 vs 解析数 vs 去重后数"三数对账。
- 🟡 **第三方订阅地址常是"不可猜"的 hash 路径**：`smartcalendars.ai` 的 `.ics` 既不在首页也不在
  sitemap（`/cal/<slug>.ics` 会 404），真实地址藏在 `…/en/feeds/<feed-slug>` 页面的 **RSC/JSON 载荷**里
  （`webcal://…/cal/<64 位 hash>.ics`）。**做法**：抓 feed 详情页 → 正则取 `[^"']*\.ics` 与 `webcal:`；
  并把"若失效如何重新找"写进代码注释（本项目写在 `src/econ_calendar.py` 的模块 docstring）。

## 数据层（结果值层 TradingView，2026-09-19）

- 🔴 **`unit` 可能是「键缺失」而不是值为 `null`**：实测 TV 响应 **4658 条里只有 2072 条带 `unit` 键**
  （值只有 `%` / `$` / `cf`），美国 `CPI` 与 `Non Farm Payrolls` **连键都没有**；而样例记录（`LMI`）
  恰好也没有 ⇒ 照抄"示例记录的字段表"写解析会以为 `unit` 恒存在。
  **判据**：接外部源时逐字段统计"键存在率"，别只看一条样本；取值一律 `.get()`，**缺失即 `None`，
  绝不给默认单位**（本项目计划里"CPI 是指数水平 334.98、非农是 162"正是靠这条才没被写成 `334.98%`）。
- 🔴 **同一 `(日期, kind)` 在值侧有"首选标题 / 备选标题"两级时，只靠排序会由上游返回顺序决定**：
  PCE 的 `Core PCE Price Index MoM`（首选）与 `PCE Price Index MoM`（备选）**同日并存**，两者的
  择优键（forecast/actual/previous 非空、date）**完全并列** ⇒ 排序后取首 = 取到**上游先返回的那条**
  （实测取到了备选，值也就跟着错）。**修法**：过滤后先取 `min(候选的首选序号)` 把备选整批排除，
  **再**排序择优；备选只在首选当天缺失时才启用。**判据**：凡是"多个可接受键 + 择优规则"的匹配，
  都要问"并列时谁决定"，答案不能是"上游顺序"。
- 🔴 **值层窗口必须与"消费它的页面窗口"取齐，否则三方对账永远不等**：值层只 enrich 窗口内的骨架行，
  而断言写成"sqlite 全库 `actual is not null` 计数 == 页面渲染数"时，**全库计数必然大于页面**（实测
  29 vs 22）。**修法**：① 把值层窗口定成与页面默认窗口同值；② 断言里的 sqlite 侧**显式加同一 range**。
  两条至少做一条，且要在注释里写明"改窗口必须同步改断言范围"。
- 🔴 **`CREATE TABLE IF NOT EXISTS` 对既有库一列都不会加，而读侧可能把这件事静默吞掉**：
  `econ_events` 加 8 列后，`web/app.py` 的启动恢复链在**库非空时提前 `return "db"`（不调 `init_db()`）**
  ⇒ 若某个部署副本还是旧的 `.db`，`query_econ_events` 会抛 `no such column`，而它**把
  `sqlite3.DatabaseError` 吞成 `[]`**（"损坏容错"纪律的副作用）⇒ 表现为 `/timeline` 变成
  "近 90 天内没有事件"，**不是报错**。**修法**：列先行（写库方 `init_db()` 先跑迁移）；
  本项目的 `.db` 已入 git，迁移随提交上线，故按"列先行"即可；**排障判据**：页面空但 API 200 时，
  先 `PRAGMA table_info(econ_events)` 数一遍列，不要先怀疑数据源。
- 🟡 **`ALTER TABLE ADD COLUMN` 迁移的独立验收要"对备份逐行比"，且要排除 `fetched_at`**：
...
- 🔴 **评估"凭据泄露"时，第一步是查仓库可见性，不要假设**：`docs/system-overview.md` 的 G1 原文
  ...
  `gh repo view <owner>/<repo> --json privacy,visibility` → **`PUBLIC`** ⇒ 凭据是**全网可克隆**，
  严重等级差一个量级。**判据**：任何"凭据/密钥"类风险，先落一条可见性实测再定级。
- 🔴 **止损只能靠"后台吊销"，删代码不是止损**：代码一删只是停止继续扩散；
  只要旧凭据在服务端仍然有效，**已获得它的人随时能用**。处置顺序必须是
  **先吊销/轮换 → 再改代码**（这也是为什么这类任务的 Step 0 属于用户、且不能由执行者代做）。
- 🔴 **平台不会替你兜底**：实测 `gh api repos/<o>/<r>/secret-scanning/alerts` → **0 条告警** ——
  GitHub Secret Scanning 只认它支持的那些凭据类型，**企业微信 bot 凭据不在其中**。
  **判据**：不要依赖"平台会告警/会自动吊销"来降低处置优先级。
- 🔴 **改凭据时**不要用"逐行编辑"**：逐行编辑要求把**凭据原文**写进工具调用/对话记录/补丁脚本参数
  ⇒ 等于把刚泄露的凭据又抄一份到日志里。**修法**：用一次性脚本做"定位 + 替换"，
...
- 🟡 **防再犯守卫的四个坑**（`tests/test_env_util.py`，全部实测踩过）：
  ...
  ② **排除 `tests/`** —— 测试里的假凭据是合法的（本项目 `verify_ui.py` / `test_web.py` 有鉴权夹具），
  扫进去必然假红。
  ③ **守卫必须自检两件事**，否则是"空转的假绿"：正则**真的能命中**（用假值喂样本）+ **扫到了足够多文件**
  （断言 `n_files >= 20`；扫 0 个文件也会"通过"）。
  ④ **失败信息只报 `文件:行号:变量名`** —— 报值等于把凭据又输出到 CI 日志。
- 🟡 **`.env` 值里"引号内的空白"刻意不 trim**：`KEY="  spaced  "` 读出来是 `  spaced  `（保留内部内容）。
  与 `src/news_fetcher.py:_load_env` 同语义，也是本项目的既定取舍：**不 mangle 敏感值**
  —— 口令里出现前导/尾随空格是合法的，静默 trim 会让"配了却登不上"极难排查
  （同 `web/app.py` 的 `MP_AUTH_PASS` 不做 strip）。

## 验收（断言"预置位"必须验证生效并可重试，2026-09-19）

- 🔴 **"先手动把状态摆到位、再采样"的断言，必须验证这一次摆位真的生效**：`B-6b`（告警 ticker 回绕）
  的探针会先把滚动容器顶到「周期 − 2px」再采样帧序列。但页面自己的 TTL 刷新会**重建 ticker DOM**，
  而 `renderAlerts` / `renderNews` 里都有 `body.scrollTop = 0` ⇒ 若这次重建正好落在"摆位"与"首帧采样"之间，
  预置位被冲成 0，采样窗口里看不到回绕 ⇒ **假红**（实测：`first=777 → 0/1`、`wrapIdx=-1`、`drops=[]`）。
- **判据（怎么确诊是竞态而不是真回归）**：① 同一断言在同一页面连跑 3 次，**通过/失败交替**；
  ② 把"加载后静默期"从 1.5s 拉到 5s 就 3/3 通过（重建落在窗口外）；
  ③ 与改动前隔离副本（`git archive HEAD`）跑同一断言，A/B 有差异但**页面行为本身没变**（周期/速率一致）。
- **第一版修法（不放松判据）**：让探针**重试摆位** —— 起始帧若发现位置没落到位，就再摆一次，
  且**不计入样本**，并把重试次数带回结果（`armTries`）打印出来。**判据本身一条都没改**
  （仍要求"观察到回绕 + 回绕后至少 3 帧 + 前进 > 0.5px"）。
  **验证方法**：主动注入一次"采样前把 scrollTop 归零 + 累加器归零"（等价于一次重渲染）⇒
  修前必红、修后 `armTries=1` 且通过。
- 🔴🔴 **第二版修正（2026-09-19 当天再次翻车，根因比上面更深）**：上面的"重试 20 帧"**不够**，
  因为**真正的杀手是被观测对象自己也在动**。实测逐帧诊断（`after-arm 931 → f7 933 → f8 0`）：
  探针把预置点放在 `period − 2`，**距回绕点只有 2px**，而滚动器持续运行
  （16px/s ÷ 60fps ≈ 0.27px/帧）⇒ **约 8 帧后它自己就回绕**。
  旧实现的"首样本"留给第一次 `requestAnimationFrame`，于是 arm → 首帧之间只要有 **>2px 的延迟**
  （布局 / GC / 低帧率 / TTL 重建），位置已回绕归零，**被旧判据 `out.length === 0 && cur < start-1`
  误判成"摆位没生效"** → 重新摆 → 再被抢先 → **20 帧耗尽** → 假红。
- **判别要点（这次差点又当成"随机抖动"放过去）**：① 失败签名恒为
  `first=0 drops=[] armTries=20`（**首次采样前就已归零**，而非采样中途停住）；
  ② 与改动前隔离副本 A/B：基线 `first=777 armTries=2` 绿、工作区同步红 ⇒ **确实是本次改动触发的**；
  ③ 但"被本次改动触发"**≠"本次改动有 bug"** —— 改动只是让布局更快、把延迟推进了那个 2px 窗口。
- **第二版修法**：① `arm()` 之后**同帧同步**读一次作首样本（同一帧内读回必等于预置值，不可能被抢占）；
  ② 重试判据从"**还没采样**"改为"**还没观测到回绕** + 位置掉回起点附近"（≤3 轮重采），
  这样正常回绕不再被误判成摆位失败，而 TTL 重建冲掉位置时仍会重试。判据依然未放松：
  **始终没见过回绕照样判红**。修后连跑 2 次全绿、`armTries=0`。
- ⚠️ 引申（原）：凡是"**先设状态再观察**"的断言（滚动位置、`localStorage`、主题属性、假 now、mock 时序），
  都要问一句"这个状态会不会被页面自己的异步任务覆盖"；会的话，**在观察窗口内确保它成立**再断言。
- ⚠️ 引申（新）：还要再问一句"**这个状态会不会自己往前走**"。若被观测对象是**持续运行**的
  （动画、滚动、计时器、心跳），则"摆位 → 观察"之间的**时间余量**就是一个隐含常量
  （本次是 `2px ÷ 0.27px/帧 ≈ 8 帧`）—— 摆到"临界点前 N 像素"这种写法必须**同时把 N 算够**，
  或者干脆**同帧采样**，否则它会在任何时序扰动下翻车。

## 工具链（补丁经 shell heredoc 传递会吃掉反斜杠转义，2026-09-19）

- 🔴 **不要把含正则的代码用 `python - <<'PY'` 这类"读脚本再写文件"的方式打补丁**：本次给
  `src/econ_calendar.py` 写 `_period_zh` 时，正则里的 **B 边界转义在传递后退化成了不可见的退格符（0x08）**，
  于是 `re.search(...)` 静默失配 —— **中文标题里的数据期全部丢失**，而表观上（语法、编译、单测骨架）
  都完全正常，只有逐条打印真数据才发现。**症状判据**：解析函数"看起来对"但结果为空/恒定；
  `grep` 看不到该转义、`repr()` 能看到控制字符。
  **自检一句话**：`python -c "import pathlib;print(pathlib.Path('x.py').read_text().count(chr(8)))"` 应为 0。
