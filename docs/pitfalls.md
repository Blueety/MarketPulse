...
- **snippet 首尾还带「发布时间/栏目名」元数据，切句管不到**：实测残留 `10 9月 2026, 09:13 情报报告称…`（首部）、`…揭开背后线索 09-09 08:22 9月9日财经早餐：…`（尾部）。另 `[1]`/`[1.3.3]` 参考文献标记会在截断后留下半截中括号（`…30% […`）。**脚注已修**（`re.sub(r'\[\d+(?:\.\d+)*\]', '', ...)`），元数据碎片**未修**（需另加模式匹配，见 journal 待确认项）。
