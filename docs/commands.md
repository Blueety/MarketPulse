...
| `venv/Scripts/python scripts/backfill_history.py [--dry-run] [--symbols SH,SZ]` | 历史数据回填（补 1Y）：走 `_yahoo_chart_get` 双主机轮换、按符号所属市场时区归档日期、**只新增不改既有行**、经 `merge_history` 按 date 合并、收尾按 date 升序重排并断言（**不碰 `last_values.json`**）；`--dry-run` 只打印计划不写盘。**执行前必须先备份 `data/history.json`**；前置：`history.retention_days=365`，否则新增行会被 `HISTORY_MAX` 立即裁掉 | 需要补齐 1Y 历史 / 改了回填逻辑后 |
