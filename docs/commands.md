...
| 改 `config.json` 阈值后怎么验（2026-09-20） | ① 开 `/backtest`（nav「阈值回测」）——该页**故意不缓存**，刷新即见新口径与新数字；② `curl -s localhost:<port>/api/backtest | python -m json.tool | head -40` 看 `threshold_config` 与 `stats`；③ 与 CLI 对打：`venv/Scripts/python scripts/backtest.py` 的 md 报告数字应与 `/api/backtest` **同源一致**（两者共用 `src/backtest.py`）；④ `pytest tests/test_backtest.py tests/test_web.py -q -k backtest` |
| 设置页（2026-09-20） | nav「设置」→ `/settings`：改阈值/动态三参数/自选 → 保存即生效（POST → 白名单校验 → 备份 → 原子写 → `reload_config_snapshots()`）；被 `ALERT_THRESHOLD_*` env 覆盖的键会标注「改了也不生效」；Railway 上只读（POST 403）。契约测试 `pytest tests/test_settings_store.py tests/test_web.py -q -k settings` |
| 组合盈亏（2026-09-20） | 设置页自选表格录**成本价（可选）** → 首页自选卡新增「持仓盈亏」列；未录成本的行显示 `—`（**不是 0.00%**）；卡头概览 = 有成本标的**等权平均**并标注「未含份额」（无 shares ⇒ 算不出金额/市值/加权收益）。契约测试 `pytest tests/test_web.py -q -k pnl`、验收 `PF-*` 组 |
...
| ⚠️ `scripts/migrate_to_sqlite.py` —— **已归档，勿再执行**（BUG-005） | 三十一期迁移**已完成**，脚本已移档到 `tasks/2026-09-24-qa-bughunt/legacy/migrate_to_sqlite.py`（留下一段说明与守卫，`main()` 无条件拒绝运行并退出码 1）。它**没有 `--dry-run`**：任何调用都会 `DELETE FROM history` 后从旧 `data/history.json`（末行 2026-09-11）全量重灌 ⇒ 裸跑会把生产库 272 个日期塌回旧快照。故**不再提供命令** | —（禁止执行） |
