...
**6 个页面路由 / 13 个 JSON 端点**（12 个业务 API + `/healthz` 健康检查）
...
| `GET /settings` | **设置**（2026-09-20；「设置」占位转正；本地可写 / Railway 只读） |
| `GET /settings` | **设置**（2026-09-20；「设置」占位转正；本地可写 / Railway 只读） |
...
| `GET /api/settings` | 设置页数据与保存（GET 恒 200；POST 白名单校验 → 备份 → 原子写 → **重载快照**；Railway 403） |
| `GET /api/settings` | 设置页数据与保存（GET 恒 200；POST 白名单校验 → 备份 → 原子写 → **重载快照**；Railway 403） |
