...
| # | 问题 | 影响 | 建议 |
|---|---|---|---|
| **G1** | **硬编码凭据**：`src/wecom_channel.py` / `wecom_sdk.py` / `wecom_ws.py` 三个文件都硬编码企业微信 `BOT_ID` + `SECRET` | 仓库会 push 到远端 → **凭据泄露** | 改走 `.env` + 既有 `_load_env()` 模式（`src/news_fetcher.py:22` 有现成实现）；**并在企业微信后台重置已泄露的凭据** |
