...
单文件 Python 脚本项目。每个交易日收盘后运行 `daily_report.py`，从 Yahoo Finance 与 FRED 拉取 VIX / VXN / MOVE 三个波动率指数，生成 Markdown 日报并缓存当日值，交由 Hermes 读取并推送到 QQ 机器人。**脚本自身不包含推送逻辑。**
...
| 数据源 | VIX/VXN 用 Yahoo，MOVE 用 FRED | 对应任务 prd 约定；Yahoo 失败可备选 FRED 的 VIX | 2026-08-29 |
...
- 安全边界: 敏感信息只走环境变量；不在代码或 git 历史里写入 `FRED_API_KEY`。
