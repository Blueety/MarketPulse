...
- `scripts/backfill_history.py`: 历史数据回填（补 1Y，2026-09-11）：`--dry-run` 只打印计划；走 `fetcher._yahoo_chart_get`（双主机轮换）、按符号所属市场时区归档日期（A 股→上海 / 其余→美东）、**只新增 date 不存在的行**（既有 `None` 是休市/未收盘的真实语义）、丢弃「仅 BTC 有值」的非交易日（BTC 7×24 的周末 bar）、经 `analyzer.merge_history` 按 date 合并、收尾**按 date 升序重排 + 断言**（`merge_history` 只 append 不排序）、**不碰 `data/last_values.json`**。前置：`history.retention_days=365`。**执行前先备份 `data/history.json`**。`seed_history.py` / `seed_history_market.py` 为早期脚本，**勿再使用**（小写键覆盖 `last_values` + 整行覆盖 `history`）。
