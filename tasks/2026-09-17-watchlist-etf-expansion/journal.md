...
## 追加：半导体 ETF 补位（23:0x，用户指派）

- 背景：中证半导体产业指数 `931865` 无可用源（新浪 KeyError / 东财被拒）→ 用户采纳"换对应 ETF"方案。
- **实测 4 只候选全部可取**（走已改好的 ETF→东财通道，含当日收盘）：
  `512480.SS` 半导体ETF国联安 1.001 / `159813.SZ` 半导体ETF鹏华 0.715 /
  `159995.SZ` 芯片ETF华夏 1.114 / `512760.SS` 半导体芯片ETF国泰 1.06。
- **按用户点名收录 2 只**（512480、159813）⇒ 自选 **11 只，11/11 取数成功**，快照已重写。
  ⚠️ 两只暴露高度重叠（中证全指半导体 vs 国证半导体芯片），留一只可随时说。
- `ICON_CHARS` 补 `半` / `芯`；图标色仍按配置序取（11 行 / 10 色调色板 ⇒ 有两行同色，属预期）。
- **几何复测**（4 视口）：`docH` 1920=**1216（与 9 只时完全相同，`≤1240` 护栏安全）**、1600=1168、
  1280=2170、375=2713；**四档零横向溢出**；11 行降序正确、`distinct_colors=10`。
- `verify_ui.py` 本次 `FAILED: 1 条`，仍是 **`B-6b`**（签名 `(-1, 0, 382)`，`wrap_idx=-1`）。
  ⚠️ **新增观察**：该红出现频率升高（近 4 次 3 红）；`first=0`（≠ 预期的 `period-2`）说明
  **`wrap_js` 的 seek 未生效**，而今日 `alerts/` 新增了一份告警文件（列表内容变化）——
  怀疑 `period = items[n].offsetTop - items[0].offsetTop` 在**当前告警条数**下退化，
  使采样器从 0 起采、200 帧内走不到回绕点。**仍与自选改动无关**（`#alert-list` vs `#watchlist-body`），
  但它已实质阻塞 `ALL PASSED`，**建议尽快单独修探针**（修法见上节）。
