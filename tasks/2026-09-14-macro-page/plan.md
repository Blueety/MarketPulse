...
| **利率** | ^TNX（10Y）变化方向 | 5 日变化幅度分档 ⚠️ **必须用「百分点差」`_chg_abs`（4.00→4.30 = +0.30pp），不能用百分比变化**（后者得 7.5%，与 0.10/0.25 的阈值量纲不符 → 任何变动都顶格）|
...
⚠️ **键大小写必须统一（执行者 2026-09-14 抓到的真 bug）**：

`/api/macro` 的 `trend.series[].key` 是 **`sym.lower()`**（如 `"dx-y.nyb"` / `"^tnx"`），而相关性对若按**展示符号**写（`"DX-Y.NYB"` / `"^TNX"`）→ **6 对全部取不到值 → 全是 `r=None`**，页面表现为「**永远样本不足**」——**静默失效，不会报错**。

**正确做法**：查表前**先转小写**。

📌 这其实是**项目既有纪律**：`analyzer._HISTORY_KEYS = frozenset(s.lower() for s in SYMBOLS)` —— history 一律小写键。本 plan 原文违反了这条已知约定。
...
**验证**：`pytest tests/ -v` **全绿** —— ⚠️ 但**原 plan 的「默认值不变 → 不应有任何失败」假设是错的**（执行者 2026-09-14 实测更正）：

`tests/test_phase24.py` 有 4 个**单参 lambda 桩**（`lambda stocks: ...`）。给 `fetch_watchlist` 加 `range_` 关键字参数后，调用 `fetch_watchlist(cfg, range_="5y")` 会 `TypeError: unexpected keyword argument`。

**正确处理（已采纳）**：**生产代码正确、桩跟随** —— 改桩为 `lambda stocks, range_="2y": ...`，**不删除任何断言**；并**新增一条 range 透传护栏测试**（防将来有人漏传）。

⚠️ 教训：改函数签名前**必须先 grep 所有 mock 桩的签名**（`monkeypatch.setattr` 的 lambda），不能假设"默认值不变就零影响"。
...
⚠️ **`^TNX` 的口径 —— 已实测更正（2026-09-14）**：

**Yahoo 返回的就是百分数，不需要任何换算。**

```text
^TNX  n=1255  first 2021-09-14 = 1.2770   last 2026-09-14 = 4.9850
```

`1.277` 对应 2021-09 美国 10Y 收益率 ≈1.28%（历史事实吻合）→ 值本身就是 `%`。

❌ **原 plan 写的「收益率 × 10，必须除以 10」是错的** —— 它基于"业界常识"而非实测，若照做会把 4.985% 显示成 0.4985%，**正好制造出与预期相反的错误**。

**处理方式（执行者提出，已采纳）**：**量级自适应** —— `|v| > 20` 才 ÷10（正常 10Y 收益率不会超过 20%），兼顾上游万一换成 ×10 口径时的自愈。
- ⚠️ 必须配 `log.warning`（**不允许静默校正** —— 见 `docs/pitfalls.md` 的「静默降级会掩盖故障」）
- ⚠️ **必须加单测锁定当前口径**（`4.985 → 4.99%`，不换算），防将来有人"顺手"加 ÷10
- ⚠️ 只放在**显示层**（`macro.js` 的单一函数，如 `toYieldDisplay(v)`），不要散落多处
- 📌 相关性计算**不受影响**（Pearson 是尺度不变的，×10 不改变 `r`）→ 不必进数据层
...
- `docs/pitfalls.md` 追加：
  ...
  2. **`^TNX` 口径已实测澄清**：Yahoo 返回的就是百分数（`4.985` = 4.985%），**不要换算**。用"业界常识"代替实测会写出反向的转换规则。
...
| **R3** | ~~`^TNX` 收益率×10~~ → **已实测更正**：Yahoo 返回的就是百分数 | ✅ **已澄清** | **不换算**；量级自适应（`|v|>20` 才 ÷10）+ `log.warning` + 单测锁定口径。原 plan 的"必须除以 10"是**基于常识而未实测的错误**，照做会显示成 0.4985% |
...
- [x] **`^TNX` 口径已实测澄清**：Yahoo 返回百分数，**不换算**；量级自适应 + `log.warning` + 单测锁定（R3）
