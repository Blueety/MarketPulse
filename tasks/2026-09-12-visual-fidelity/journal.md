...
- **四追记（用户放大反馈"图片不清晰，不是尺寸问题"）→ 图标级简化素材**：根因是**细节密度**，不是加载/尺寸——twemoji 全保真旗在 20px 画布物理装不下：US 角块 24 颗星每颗 0.4 物理像素（亚像素噪点糊成灰斑）、CN 4 颗小星各 0.7px；且 twemoji 旗面只画在 viewBox y=5..31（自带 28% 上下留白，20px 框里实际旗面仅 14px）。处置：重写两枚 SVG 为**图标级简化**——全出血 rect（+28% 旗面）、条纹 13→7 道（每道 2.9px，AA 后仍清晰）、删亚像素星（US 角块留 5 颗 1.6px 白点提示星区、CN 留单颗大星 3.6px 半径），配色/圆角交给 CSS 不变。验收：逐图标像素普查 6/6 OK（US 角块 252→312、条纹 321→492；CN 红底 794→1318，星 84 单颗清晰），naturalWidth 全 150。教训：**小尺寸图标用"细节做减法"换清晰度**——全保真素材在 ≤24px 下必然发糊，1px 以下的 path 是噪点不是细节；判断"素材没渲染"前先 Counter 看真实色值（本任务探针三次自伤：阈值错配/px 变量复用/卡序映射错）。
- **五追记（用户参照效果图定稿）→ 尺寸回退 16px + 图形素材**：①尺寸 20→16 全量回退（.ico/两处 bar-row/F-1e 同步回 16×16，scrollHeight 回 1216）；②概览 4 张非市场卡（美元/10Y/黄金/原油）参照效果图换**圆形图形素材**：新增 `web/static/icons/{dollar,bond,gold,oil}.svg`（teal $ / 灰银行 / 琥珀金块 / 深色油滴，`<circle>` 全出血 + 白色 path/`<text>` 符号，每枚 <300B）；美股/A股保留旗。app.js：`iconAssetHtml(colorVar, name)`（底层品牌色 + 兜底 char，img onerror 露出）、OVERVIEW_CARDS 加 `icon` 字段。F-8 改为「旗 US×1 + 旗 CN×1 + img×6」，F-1e 回 16×16，先红（2 FAIL）后绿（ALL PASSED）。像素普查：4 枚圆形素材 bg 色 570~699px 全命中；CN 旗黄星 74px 为 16px 正确量级（探针阈值又忘随尺寸重标，判读时警惕）。SVG `<text>` 走系统字体（Arial），<img> 渲染无外链可用。
