...
> `style.css` 与 `verify_ui.py` 已整体回退至 `26da86f` 纯光斑验收态。详见文末「追记」。
> 当前生产 CSS **无纹理层**；pitfalls 中的机制教训仍有效。
...
- **决策**：纹理 alpha `.05 → .04 → .03` 两轮调淡后用户仍觉「丑」，选「去掉纹理——回退纯光斑」。
  标定数据（.022 不可见 / .05≈峰谷差12）只保证「可辨」，不保证「好看」——观感取舍归用户。
- **操作**：`git checkout 26da86f -- web/static/style.css tasks/2026-09-11-frontend-bento-redesign/verify_ui.py`
  （还原前先 `git log --oneline 26da86f..HEAD -- <paths>` 确认中间只有本任务提交碰过这两文件；
  还原后 `git diff 26da86f -- <paths>` 为空、行数 545/672 与任务基线一致）。
- **验证**：`verify_ui.py` → **ALL PASSED / EXIT=0**（dark/light 均 body 层=2、backdrop 11/11、
  scrollH@1920=1216）；像素取证 §6.2 窄条单列峰谷差 **中位 2**（无纹理基线 1，有纹理时 10）
  → 纹理确认消失；卡内残差 0.00。
