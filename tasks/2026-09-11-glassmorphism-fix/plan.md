...
| 侧栏 | 与页面**同层、透明**，仅靠 1px 右分隔线区分；**底部无主题切换按钮** | `background: var(--bg-primary)`（不透明）+ 有主题切换按钮 | 侧栏改透明（**不参与**玻璃层，避免 R19）；主题按钮去留见 §12 第 7 项 |

#### 4.6.4 promo 卡校准（承接原 §12 第 3 项）

效果图的「全球市场动态」是**真实照片底**（黄昏雪山 + 蓝紫天空）+ 底部暗渐变遮罩 + 白字 + 「查看分析 →」。

- **不再推荐纯 CSS 渐变近似**：照片感无法用渐变还原。
- 建议：新增 **1 张 WebP**（约 900×300，压缩后 **≤120 KB**）+ `linear-gradient(180deg, rgba(6,10,16,.10), rgba(6,10,16,.78))` 遮罩保证文字对比度；**CSS 渐变作兜底**（图未加载时不塌）。
- 代价：仓库新增二进制资源（旧 plan §13 曾禁止）。**见 §12 第 3 项**；若不允许则退回 CSS 抽象，视觉差距明显。
...
- 按 §4.4 改 `.card.promo`（暗调氛围渐变 + 玻璃边 + blur）。
- **验证**：断言 6 PASS（promo 的 `borderTopColor` alpha > 0）；目视不再是亮蓝青块。
...
| 类型 | 文件 | 规模 |
|---|---|---|
| 修改（主要） | `web/static/style.css` | 412 → ≈470 行（新增 glass token 双套 + 氛围层 + 卡片玻璃化 + 2 处半透明 + promo 改造 + `@supports` 降级；删除 `--card-glow`/`--card-shadow` 相关引用） |
...
3. **【需确认】promo 卡形态**：效果图是**真实照片底**（黄昏雪山），CSS 渐变无法还原。
   - ① **推荐**：新增 1 张 WebP（≤120 KB）+ 暗渐变遮罩 + CSS 渐变兜底（最接近效果图）；
   - ② 若拒绝新增二进制资源：退回纯 CSS 暗调抽象（视觉差距明显）。
...
7. **【需确认】侧栏主题切换按钮**：效果图侧栏底部**没有**主题切换（只有「市场已开盘 / 北京时间」），当前实现有。移除会失去唯一主题入口 —— 保留还是按效果图移除？
