...
1. **【阻塞精度】效果图需重新发送**：原 `image.png` 在本会话已不可读（早期附件失效）。玻璃的**强度**（blur 半径、面板 alpha、光斑饱和度）与 **promo 卡是否要真实照片底**，需重发效果图才能逐像素贴近。
   - 缺图时的默认动作：按 §4.2 建议值实现，并在 `journal.md` 标注「参数待与效果图比对微调」（R23）。
2. **强度档位**：轻薄（`blur(12px)` / α≈0.03 / 光斑很淡）↔ 厚重（`blur(24px)` / α≈0.07 / 光斑明显）—— 效果图偏哪一档？
3. **promo 卡形态**：
   - ① 纯 CSS 暗调氛围（延续旧 plan §13「不新增二进制资源」）；
   - ② 允许新增一张暗色氛围图（更接近效果图，但引入 ~200–400 KB 二进制）。
4. **顶栏 / 侧栏是否也玻璃化**（Step G-6）：会引入 R19（sticky + blur 残影）风险，收益是效果图里最明显的玻璃线索。
5. **验收脚本位置**：本次约定**扩展现有** `tasks/2026-09-11-frontend-bento-redesign/verify_ui.py`（单一事实来源，避免两份脚本漂移）。若希望长期沉淀为仓库级工具，可改放 `scripts/verify_ui.py`（需同步改其 `ROOT = parents[2]` → `parents[1]`）——**本次建议不动，避免无关 churn**。
