...
处置分三步：① 后台吊销旧凭据（用户侧，唯一真止损）；② **代码里不再有凭据字面量**（本模块）；
③ **防再犯守卫**（`tests/test_env_util.py` 扫描仓库源码，禁止再次出现硬编码凭据）。

> **2026-09-20 更新**：用户决定**不再使用企业微信通道** ⇒ `src/wecom_*.py` 三模块与
> `scripts/wecom_service.bat` **已删除**。本模块与守卫**刻意保留** —— 它们不是 wecom 专属：
> 前者是"读敏感配置"的单点实现（将来任何需要凭据的模块都该走它），
> 后者保护的是**整个仓库**。⚠️ 当前本模块**无消费者**（原消费者已删），保留属有意为之。
